...
struct PeerClientEntry {
    seed_peer_target: String,
...
impl Builder {
    ...
    pub async fn build(self) -> Result<Proxy> {
        ...
        let proxy = Proxy {
            ...
            request_seed_peers: DashMap::new(),
        };
...
pub struct Proxy {
    ...
    /// request_seed_peers records successful seed peer targets observed for each request URL.
    request_seed_peers: DashMap<String, Vec<String>>,
}
...
impl Proxy {
    ...
    async fn client_entries(&self, request: &GetRequest) -> Result<Vec<PeerClientEntry>> {
        ...
        for peer in seed_peers.iter() {
            ...
            client_entries.push(PeerClientEntry {
                seed_peer_target: format!("{}:{}", peer.ip, peer.proxy_port),
                entry: client_entry,
            });
        }
    ...
    async fn try_send(&self, request: &GetRequest) -> Result<reqwest::Response> {
        ...
        for (index, peer_entry) in entries.iter().enumerate() {
            match self.send(&peer_entry.entry, request).await {
                Ok(response) => {
                    self.check_seed_peer_target(request, &peer_entry.seed_peer_target);
                    return Ok(response);
                }
                Err(err) => {
                    error!(
                        "failed to send request to client entry {:?}: {:?}",
                        peer_entry.entry.client, err
                    );
    ...
    fn check_seed_peer_target(&self, request: &GetRequest, seed_peer_target: &str) {
        let seed_peer_targets = match self.request_seed_peers.entry(request.url.clone()) {
            DashMapEntry::Occupied(mut entry) => {
                if entry.get().iter().any(|target| target == seed_peer_target) {
                    return;
                }

                entry.get_mut().push(seed_peer_target.to_string());
                entry.get().clone()
            }
            DashMapEntry::Vacant(entry) => {
                entry.insert(vec![seed_peer_target.to_string()]);
                return;
            }
        };

        warn!(
            "dragonfly sdk request url added seed peer: request_url={} new_seed_peer={} seed_peers={:?}",
            request.url, seed_peer_target, seed_peer_targets
        );
    }
